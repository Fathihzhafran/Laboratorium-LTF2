...
/* hitung nilai huruf berdasar batas bA, bB, bC, bD */
 char nilaiHuruf(float nilaiAngka) {
// … gunakan pencabangan untuk menentukan nilai huruf
if (nilaiAngka <=bD) {
  return 'E';
}
else if (nilaiAngka <= bC && nilaiAngka >= bD) {
  return 'D';
}
else if (nilaiAngka <= bB && nilaiAngka >= bC) {
  return 'C';
}
else if (nilaiAngka <= bA && nilaiAngka >= bB) {
  return 'B';
}
else {
 return 'A';
...
void cetakData() {
 ...
 sprintf(buffer, "%8ul %-15s %6.2f %c", nim, nama.c_str(), nilai_angka,
 nilai_huruf);
